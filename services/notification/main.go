...
func main() {
	...
	// 加载配置
	cfg, err := config.LoadConfig("notification")
	if err != nil {
	...
	// 初始化存储库
	notificationRepo := repository.NewRepository(db, logger)

	// 初始化服务
	notificationService := service.NewNotificationService(notificationRepo, redisClient, logger)
	...
	// 启动服务器
	go func() {
		logger.Info().Msgf("通知服务启动在端口 %s", cfg.Server.Port)
